...
echo "========================================"
echo "Starting Pepper Full System - headless"
echo "========================================"
...
if [ -z "${OPENAI_KEY}" ] || \
   ...
   [ "${OPENAI_KEY}" = "PASTE_YOUR_OPENAI_API_KEY_HERE" ]; then
...
if ! ssh -o BatchMode=yes -o ConnectTimeout=5 "${PEPPER_USER}@${PEPPER_IP}" "echo pepper_ssh_ok" >/tmp/pepper_ssh_check.log 2>&1; then
    ...
    echo "Fix it with: ssh-copy-id ${PEPPER_USER}@${PEPPER_IP}"
    cat /tmp/pepper_ssh_check.log
    exit 1
fi

echo "[1] Validating Pepper action / animation JSON..."
python3 -m json.tool \
    "${WORKSPACE}/src/pepper_social_skills/config/pepper_actions.json" \
    >/tmp/pepper_actions_check.json
python3 -m json.tool \
    "${WORKSPACE}/src/pepper_social_skills/config/pepper_naoqi_animations.json" \
...
LAUNCH_LOG="${LOG_DIR}/pepper_full_system_$(date +%Y%m%d_%H%M%S).log"
PID_FILE="${LOG_DIR}/pepper_full_system.pid"

echo "[3] Launching ROS2 system in background..."
echo "    Log: ${LAUNCH_LOG}"
nohup bash -lc "
  cd '${WORKSPACE}'
  source /opt/ros/humble/setup.bash
...
echo "[5] Waking up Pepper and opening tablet dashboard..."
...
echo "Stop with:  kill \$(cat ${PID_FILE}) ; pkill -f pepper_full_system.launch.py"
echo "========================================"
