  ...
  try {
    ...
    if (helpers::driver::isNaoqiVersionLesser(naoqi_version_, 2, 8)) {
      try {
        ...
        std::cout << "[diag] ALBodyTemperature notifications enabled" << std::endl;
      } catch (const std::exception& e) {
        std::cerr << "[diag] setEnableNotifications skipped: " << e.what() << std::endl;
      }
    }
  } catch (const std::exception& e) {
    std::cerr << "[diag] ALBodyTemperature unavailable: " << e.what() << std::endl;
  }
}
