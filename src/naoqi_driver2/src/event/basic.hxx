    ...
    isDumping_(false)
{
  ...
  converter_->registerCallback(message_actions::PUBLISH,
                               boost::bind(&Publisher::publish, publisher_, ph::_1));
  converter_->registerCallback(message_actions::RECORD,
                               boost::bind(&Recorder::write, recorder_, ph::_1));
  converter_->registerCallback(message_actions::LOG,
                               boost::bind(&Recorder::bufferize, recorder_, ph::_1));

  try
  {
    signal_ = p_memory_.call<qi::AnyObject>("subscriber", key_);
    std::cout << "[event] subscriber created for key: " << key_ << std::endl;
  }
  catch (const std::exception& e)
  {
    std::cerr << "[event] failed to create subscriber for key "
              << key_ << ": " << e.what() << std::endl;
  }
  catch (...)
  {
    std::cerr << "[event] failed to create subscriber for key "
              << key_ << ": unknown exception" << std::endl;
  }
}
...
template <typename Converter, typename Publisher, typename Recorder>
void EventRegister<Converter, Publisher, Recorder>::resetRecorder(
...
void EventRegister<Converter, Publisher, Recorder>::registerCallback()
{
  try
  {
    ...
    std::cout << "[event] signal connected for key: " << key_ << std::endl;
  }
  catch (const std::exception& e)
  {
    std::cerr << "[event] failed to connect signal for key "
              << key_ << ": " << e.what() << std::endl;
  }
  catch (...)
  {
...
void EventRegister<Converter, Publisher, Recorder>::unregisterCallback()
{
  ...
  {
    std::cerr << "[event] failed to disconnect signal for key "
              << key_ << ": " << e.what() << std::endl;
  }
  catch (...)
  {
